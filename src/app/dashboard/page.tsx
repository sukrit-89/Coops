...
export default async function DashboardPage() {
    ...
    if (isPlatformAdmin || isCooperativeAdmin) {
      ...
      return <PageShell title={isPlatformAdmin ? "Platform Dashboard" : "Cooperative Dashboard"} description="Live operational totals from the workers and bookings you are authorized to view."><Metrics metrics={[{ label: "Total workers", value: workers.count ?? 0, icon: Users }, { label: "Active workers", value: activeWorkers.count ?? 0, icon: CheckCircle2 }, { label: "Total bookings", value: bookings.count ?? 0, icon: CalendarCheck }, { label: "Completed jobs", value: completedJobs.count ?? 0, icon: CircleAlert }]} />{!isPlatformAdmin && !cooperativeIds?.length ? <p className="mt-5 text-sm text-neutral-500">Your account is not linked to a cooperative yet.</p> : null}</PageShell>;
    }
    ...
    return <PageShell title="Your Dashboard" description="Your activity across the Coops service network.">{metrics.length ? <><Metrics metrics={metrics} />{isWorker ? <Link href={"/profile/worker" as Route} className="mt-5 inline-flex rounded-xl bg-[#0b0f1a] px-4 py-3 text-sm font-medium text-white">Edit worker profile</Link> : null}</> : <EmptyState title="Choose a platform role" body="Your account is authenticated, but it does not have a customer, worker, cooperative, or administrator role yet." />}</PageShell>;
}
